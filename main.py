...
search_entry.insert(0, "输入关键字,点击目标数据按钮")

# 创建一个自定义字体对象为黑体
from tkinter.font import Font

placeholder_font = Font(family='黑体', size=10)


def set_placeholder_style(event):
    if not search_var.get():
        search_entry.config(font=placeholder_font, foreground='gray')
        search_entry.insert(0, "输入关键字,点击目标数据按钮")
    else:
        search_entry.config(font=('STXingkai', 10))


search_entry.bind("<FocusIn>", lambda event: search_entry.delete(0, "end"))
search_entry.bind("<FocusOut>", set_placeholder_style)
            ...
            try:
                ...
                if num_to_extract <= len(selected_rows):
                    ...
                    if len(extracted_df.columns) > 0:
                        ...
                        if len(extracted_df.columns) == len(original_column_names):
                            column_names_str = ', '.join(original_column_names)
                            data_without_index = extracted_df.to_string(index=False)
                            lines = data_without_index.split('\n')
                            if len(lines) > 1:
                                data_without_first_line = '\n'.join(lines[1:])
                                data_with_columns = column_names_str + '\n' + data_without_first_line
                                result_text.config(state='normal')
                                result_text.delete(1.0, tk.END)
                                result_text.insert(tk.END, data_with_columns)
                                result_text.config(state='disabled')
                        else:
                            messagebox.showerror("错误", "提取的数据列数与原始列数不一致。")
                else:
